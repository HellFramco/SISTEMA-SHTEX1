...
class Indicadores
{
    ...
    public function conteoReferenciasExistentes()
    {
        ...
        $arreglo = array();
        $i = 0;
        $consulta = "SELECT COUNT('id_inventario') as conteo from inventarios_productos where reprogramacion=1;";
        $modules = $conexion->query($consulta)->fetchAll();
        foreach ($modules as $key) {
            return $key['conteo'];
        }
    ...
    public function conteoReferenciasPorTipo($tipo)
    {
        ...
        $arreglo = array();
        $i = 0;
        $consulta = "SELECT COUNT(id_inventario)as conteo FROM inventarios_productos WHERE tipo = '".$tipo."'";
        $modules = $conexion->query($consulta)->fetchAll();
        foreach ($modules as $key) {
            if($key['conteo']==''){
                return 0;
